...
VSPA_configuration();  % what configuration?? 
...
dual_test = 0; % why is this needed? -- based on which torque-angle curve you want (dual cam vs no dual cam)
...
if(~dual_test) 
    theta_total = deg2rad(plantar_max:0.005:dorsi_max); % ankle ROM range
    theta = deg2rad(theta_deg); % convert angles to rad
    M = interp1(theta, M_data, theta_total,'spline'); % spline interpolation of torque (M) across cont. angle range (theta_total)
end
...
%% IMPORT FAKE DATA
% import linear torque-angle relationship with stiffness at most preferred
% stiffness for flat walking
...
if(~dual_test) % when spline interpolation is being used
    length(curve_x)
