function [x,y,r,psi,theta_cam,x_center,epsilon,mu,y_off,gamma,kdelt,stroke,work_M] = forwared_model(theta_total,M,equilibrium_angle,titanium_data,dual_test)
    ...
    work_M = cumtrapz(theta_total,M); %Energy in the torque-angle curve -- computes energy stored/released by 
    % integrating torque over angle; cumulative trapezoidal integration of
    % M with respect to theta_total; instead of area under curve, this
    % takes the running integral; better to use this instead of integral or
    % trapz because it gives vector of energy stored/released at each
    % point, rather than a overall scalar answer
    figure(23)
    hold on
    if(~dual_test)
