...
Rotary_VSPA_configuration(blue, black)
...
TA_function_RotaryVSPA(perc,ta_input, zero_origin)
...
gamma_FEA_peak = [0.075,0.087,0.116,0.140,0.178,0.244,0.303];
...
%% Spring Selection fixed slider
kSpringSys = 2.8727e+06;
...
%% Spring yield check
if(max(gamma)) > polyval(stroke-x_center*1000+x_center_min,gamma_fit_FEA_peak)
...
%% Inverse Model
preload_inv = preload;
[theta_inv,delta_inv,M_ankle,theta_cam_inv,Angles,Moments,F_cam, F_cam_2, F_cam_3,ROM_index,cam_angles,F_spring,F_ankle,F_spring_vert,F_ankle_vert,L,tau_inv,gamma_inv] = inverse_model_VSPA(preload_inv,kdelt,x_center,x_center_inv,ktranslational,epsilon,mu,y_off,r,psi,gamma,dual_test);
for i = 1:length(x_center_inv)
    if(max(gamma_inv(i,:))) > polyval(stroke-x_center_inv(i)*1000+x_center_min,gamma_fit_FEA_peak)
        disp ('Warning: spring may yield at')
        disp(x_center_inv(i))
        disp(i)
    end
end
