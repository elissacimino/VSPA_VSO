...
close all
close all hidden
%addpath('C:\Users\nikkovc\Documents\CODE\MATLABFunctions');addpath('TA');addpath('TA/ottobock');addpath('TA/wearable_tech');addpath('TA/old');addpath('IMPORTS');addpath('IMPORTS/VSO_functions');addpath('IMPORTS/data');addpath('inputs')
addpath('TA');
addpath('TA/datasets/');
addpath('TA/ottobock');
...
Mpoints = [-25,-25,grid_torque,175,177,180];
%Mpoints = [-35,-30,grid_torque,145,147,150];
%thetapoints = deg2rad([-max_plantarflexion,-max_plantarflexion+30,grid_angle,max_dorsiflexion-20,max_dorsiflexion-10,max_dorsiflexion]);
thetapoints = deg2rad([-max_plantarflexion,-max_plantarflexion+20,grid_angle,max_dorsiflexion-20,max_dorsiflexion-10,max_dorsiflexion]);
...
% [curve_x curve_y] = offsetCamCurve(x,y,r,psi,theta_total,roller_radius,'offset','diff');
...
[x,y,r,psi,theta_cam,x_center,epsilon,mu,y_off,gamma,kdelt,stroke,work_M] = forwared_model_VSPA_rotaryNew(theta_total3,M3,equilibrium_angle,titanium_data,dual_test);
...
if(~dual_test)
    ...
    %length(curve_points)
    csvwrite('outputs/Variflex_rotary_cam_curve.csv', curve_points/1000)   
    csvwrite('outputs/Variflex_rotary_moment_profile.csv', M)
end
