...
theta_total = pi/180*(-max_plantarflexion:0.05:max_dorsiflexion);
equilibrium_angle = 0;

Mpoints = [-25,-25,grid_torque,175,176,177]; %trustep
%Mpoints = [-40,-35,grid_torque,144,170,180]; %variflex
%Mpoints = [-50,-30,grid_torque,144,170,180]; %AllPro
thetapoints = deg2rad([-max_plantarflexion,-max_plantarflexion+30,grid_angle,max_dorsiflexion-23,max_dorsiflexion-10,max_dorsiflexion]); %trustep
%thetapoints = deg2rad([-max_plantarflexion,-max_plantarflexion+30,grid_angle,max_dorsiflexion-17,max_dorsiflexion-10,max_dorsiflexion]); %variflex
...
k_FEA_peak = [0.519, 0.637, 1.108, 1.663, 2.505, 3.987, 5.407];
gamma_FEA_peak = [0.061 0.066 0.059 0.065 0.068 0.064 0.058];

k_fit_FEA_peak = polyfit(x_FEA_peak,k_FEA_peak,4);
gamma_fit_FEA_peak = polyfit(x_FEA_peak,gamma_FEA_peak,4);

x_new_FEA_peak = linspace(0,lead_screw,100);
k_new_FEA_peak = polyval(k_fit_FEA_peak,x_new_FEA_peak);
gamma_new_FEA_peak = polyval(gamma_fit_FEA_peak,x_new_FEA_peak);

%plot(x_FEA_peak, k_FEA_peak)
plot(x_new_FEA_peak,k_new_FEA_peak)
%plot(x_new_FEA_peak,gamma_new_FEA_peak)
...
if(max(gamma)) > polyval(gamma_fit_FEA_peak,(x_center*1000-x_center_min))
    pointBreak = find(gamma > polyval(gamma_fit_FEA_peak,(x_center*1000-x_center_min)));
    disp ('Warning: spring may yield')
    pointBreak_p = find(pointBreak<length(theta_total3)/2);
    pointBreak_d = find(pointBreak>length(theta_total3)/2);
    rad2deg(theta_total3(pointBreak(pointBreak_p(end))))
    rad2deg(theta_total3(pointBreak(pointBreak_d(1))))
end
...
if(~dual_test)
    ...
    %length(curve_points)
    dlmwrite('outputs/AllPro_curve.txt', curve_points, '\t')   
...
%         kdelt_plantar = 350;
        ...
        %kdelt = 1y0000000000000000000000000;
        spring = 'nadella';
