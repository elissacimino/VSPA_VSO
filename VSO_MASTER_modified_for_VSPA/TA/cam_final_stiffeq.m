    ...
    stiffness_trans = 2.7*stiffness_dorsi;
    stiffness_trans_2 = 0.4*stiffness_trans;
    stiffness_leveling = 0.15*stiffness_trans_2; %Nm/deg;
    stiffness_equilibrium = stiffness_dorsi; %Nm/deg
    stiffness_plantar = 0.25*stiffness_trans; %Nm/deg

    %--------------------------------
    theta_plantar = [plantar_max;-30; -15; -7.5; -5; -2.5];
    theta_dorsi = [2.5; 5; 13.5; angle_leveling];
    theta_leveling = [angle_leveling+level; angle_leveling+2*level; angle_leveling+3*level; dorsi_max];
    theta_deg = [theta_plantar;equilibrium_angle;theta_dorsi;theta_leveling];
    M_equilibrium = (theta_plantar((end-1):end))*stiffness_equilibrium;
    M_plantar = [(theta_plantar(1:end-2)-theta_plantar(end-1))*stiffness_plantar+M_equilibrium(end-1); M_equilibrium];
    M_dorsi_1 = (theta_dorsi(1:2)-equilibrium_angle)*stiffness_dorsi;
    M_dorsi_2 = (theta_dorsi(3)-theta_dorsi(2))*stiffness_trans+M_dorsi_1(end);
    M_dorsi = [M_dorsi_1; M_dorsi_2; (theta_dorsi(4)-theta_dorsi(3))*stiffness_trans_2+M_dorsi_2];
    M_level = (theta_leveling-theta_dorsi(end))*stiffness_leveling+M_dorsi(end);
    M_data = scalefactor*[M_plantar;0;M_dorsi;M_level];
    save('TA/cam_final_stiffeq')
