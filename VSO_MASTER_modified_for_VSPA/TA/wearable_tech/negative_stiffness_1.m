function negative_stiffness_1
    ...
    %Cam1
    scalefactor = 0.3;
    % neg_bias = -12/scalefactor;
    angle_leveling = 12+equilibrium_angle; %Degrees
    theta_dorsi = [1; 3; angle_leveling];

    %Cam2
    neg_bias = -2/scalefactor;
    ...
    level = (dorsi_max-angle_leveling)/5;
    %------------------------
    stiffness_dorsi = 200/rad2deg(1); %Nm/deg
    stiffness_trans = 2.7*stiffness_dorsi;
    stiffness_trans_2 = 0.4*stiffness_trans;
    stiffness_leveling = 0.8*stiffness_trans_2; %Nm/deg;
    stiffness_plantar = 0.15*stiffness_trans; %Nm/deg

    %--------------------------------
    theta_plantar = [plantar_max;-30; -15; -7.5; -2];
    theta_leveling = [angle_leveling+level; angle_leveling+2*level; angle_leveling+3*level; angle_leveling+4*level;dorsi_max];
