...
if(cam_lin)
    ROM_thresh = deg2rad(18); %(deg) dorsiflexion angle that cam forces will be evaluated at
    scalefactor = 0.65; %0.35 0.5 0.65
    dorsi_max = 40; 
    plantar_max = -40; 
    %D:40, P:-40
...
if(different_cam)
    SolidworksImport = readtable('cam_1_final.txt');
...
if(different_cam)
    ...
    x_center_new = x_center_max-((x_center_max-x_center_min)*[0 20 40 60 80 95]*(1/100))
    %x_center_new = x_center_max-((x_center_max-x_center_min)*[0, 0.40, 0.60])
    %ktranslational = [polyval(titanium,x_center_new(1)),polyval(titanium,x_center_new(2)),polyval(titanium,x_center_new(3))];
    ktranslational = polyval(titanium,x_center_new);
    x_center_new = x_center_new*mm2m;
    
    kdelt_dorsi = 1450*ones(length(x_center_new));
    kdelt_planatar =  120*ones(length(x_center_new));
%     dkp = (5000-350)/(length(x_center_new)-1);
%     dkd = (5000-1100)/(length(x_center_new)-1);
%     kdelt_dorsi = 5000:-dkd:1100;
%     kdelt_planatar =  5000:-dkp:350;
end
...
for i = 1:length(x_center_new)
    ...
    v = plot(radtodeg(theta_new),M_ankle,'Linewidth',5);
...
if(Collins)
    ...
    stiffness_collins = [130 180 240 310 400]*ROM_collins(end); %Nm/rad 130 180 240 310 400
