...
%% Insert Your Code Here
 ...
 %Plot all Gear Ratios
 figure(5)
 hold on

 plot(kneeAngle_walk_s1_i0,walking_ratio,'LineWidth',2)
 plot(kneeAngle_standSit,sit_stand_ratio,'LineWidth',2)
 plot(kneeAngle_stair_i24,upstair_ratio,'LineWidth',2)
 plot(kneeAngle_walk_s1_i10,incline_ratio,'LineWidth',2)
 plot(kneeAngle_walk_s1_in10,decline_ratio,'LineWidth',2)
 plot(kneeAngle_stair_in24,downstair_ratio,'LineWidth',2)
 xlabel('Knee Angle'); ylabel('Gear Ratio [Ankle/Knee]')
 legend('walking','sit to stand', 'upstair', 'incline', 'decline', 'downstair'); legend box off
 set(gcf,'color','w'); set(gca,'FontSize',18); set(gca,'linewidth',2);
 % ratio_set = [walking_ratio sit_stand_ratio upstair_ratio incline_ratio decline_ratio downstair_ratio];
 % knee_angle_set = [kneeAngle_walk_s1_i0 kneeAngle_standSit kneeAngle_stair_i24 kneeAngle_walk_s1_i10 kneeAngle_walk_s1_in10 kneeAngle_stair_in24];
 % knee_angle_set = [kneeAngle_walk_s1_i0 kneeAngle_stair_i24 kneeAngle_walk_s1_i10];
 % ratio_set = [walking_ratio upstair_ratio incline_ratio];
 knee_angle_set = [kneeAngle_stair_i24];
 ratio_set = [upstair_ratio];
 knee_angle_vec = min(knee_angle_set):1:max(knee_angle_set);
 gear_ratio = []
 for i=1:length(knee_angle_vec)
     indexes = find(abs(knee_angle_set-knee_angle_vec(i))<=5);
     gear_ratio(i) = sum(ratio_set(indexes))/length(indexes);
 end
 gear_ratio_poly = polyfit(knee_angle_vec,gear_ratio,4);
 knee_angle_vec_upsampled = min(knee_angle_set):0.1:max(knee_angle_set);
 gear_ratio_exo = polyval(gear_ratio_poly,knee_angle_vec_upsampled);
 % plot(knee_angle_vec,gear_ratio,'LineWidth',5)
 % plot(knee_angle_vec_upsampled,gear_ratio_exo,'LineWidth',5)




 %Plotting Angles that Result from Knee Driving
 figure(1)
 hold on
 %Walking
 % subplot(6,3,2)
 % hold on
 % plot(gait_phase,exo_ratio_walking,'Color',[255,105,180]./255,'LineWidth',3)
 % title('Walking'); xlabel('Gait Phase'); ylabel('Gear Ratio [Ankle/Knee]')
 % exo_ratio_walking = polyval(gear_ratio_poly,kneeAngle_walk_s1_i0);
 % subplot(6,3,3)
 % hold on
 % plot(gait_phase,exo_ratio_walking.*kneeAngle_walk_s1_i0,'Color',[255,105,180]./255,'LineWidth',3)
 % plot(gait_phase,ankleAngle_walk_s1_i0,'Color',[65,106,225]./255,'LineWidth',3)
 % title('Walking'); xlabel('Gait Phase'); ylabel('Ankle Angle')

 %Upstairs
 % exo_ratio_upstairs = polyval(gear_ratio_poly,kneeAngle_stair_i24);
 % subplot(6,3,8)
 % hold on
 % plot(gait_phase,exo_ratio_upstairs,'Color',[255,105,180]./255,'LineWidth',3)
 % title('Upstairs'); xlabel('Gait Phase'); ylabel('Gear Ratio [Ankle/Knee]')
 % subplot(6,3,9)
 % hold on
 % plot(gait_phase,exo_ratio_upstairs.*kneeAngle_stair_i24,'Color',[255,105,180]./255,'LineWidth',3)
 % % plot(gait_phase,upstair_ratio.*kneeAngle_stair_i24,'Color',[255,105,180]./255,'LineWidth',5)
 % plot(gait_phase,ankleAngle_stair_i24,'Color',[65,106,225]./255,'LineWidth',3)
 % title('Upstairs'); xlabel('Gait Phase'); ylabel('Ankle Angle')
 
 
 %Inclines
 % exo_ratio_inclines = polyval(gear_ratio_poly,kneeAngle_walk_s1_i10);
 % subplot(6,3,11)
 % hold on
 % plot(gait_phase,exo_ratio_inclines,'Color',[255,105,180]./255,'LineWidth',3)
 % title('Inclines'); xlabel('Gait Phase'); ylabel('Gear Ratio [Ankle/Knee]')
 % subplot(6,3,12)
 % hold on
 % plot(gait_phase,exo_ratio_inclines.*kneeAngle_walk_s1_i10,'Color',[255,105,180]./255,'LineWidth',3)
 % plot(gait_phase,ankleAngle_walk_s1_i10,'Color',[65,106,225]./255,'LineWidth',3)
 % title('Inclines'); xlabel('Gait Phase'); ylabel('Ankle Angle')

 figure(6)
 hold on
 plot(kneeAngle_walk_s1_i0,ankleAngle_walk_s1_i0,'Color',[65,106,225]./255,'LineWidth',3)
 plot(ankleAngle_walk_s1_i0,kneeAngle_walk_s1_i0,'Color',[60,179,113]./255,'LineWidth',3)
 title('Walking'); xlabel('Angle'); ylabel('Angle')
