...
addpath('IMPORTS/functions/data')
addpath('DUAL_CAMS')
...
% pf_transition = find ((radian2deg*theta_total)==-switching_angle_plantar)
% df_transition = find ((radian2deg*theta_total)==switching_angle_dorsi)
...
% thd_adjust = radian2deg*(M_dt./kdelt_dorsi);
% thp_adjust = radian2deg*(M_pt./kdelt_plantar);
% thd_magnet = transition_dorsi-thd_adjust
% thp_magnet = transition_plantar-thp_adjust

thp_magnet = rad2deg(theta_cam_fw(pf_transition)); %angles needed in solidworks model
thd_magnet = rad2deg(theta_cam_fw(df_transition)); %angles needed in solidworks model
...
psi2_offset_err = atan(curve_y2./curve_x2);
if min(diff(psi_offset_err_))<=0 || min(diff(psi2_offset_err))<=0
...
%     OrangeMomentAnkle = dydx(work_spring_inv2,theta_cam_inv2);
    ...
    if(x_center_new(i)==x_center)
        ...
        if(isempty(DropFoot_blue_index))
        ...
        if(isempty(DropFoot_orange_index))
            Angle_DropFoot_orange = plantar_max
        else
