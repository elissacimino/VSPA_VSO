...
ROM_thresh = deg2rad(27); %nikkoROM that Cam forces are evaluated at
...
res = 0.005; %0.005               %resolution, step size between data points
...
%% ToDo: Check if the curves need to flip
orange_preloaded = 1;
if(ES<0)
...
% figure(13)
% plot(theta_total,OrangeMomentIdeal)
% slope_blue_error = [];
% slope_orange_error = [];
% 
...
%         slope_orange_error = [OrangeMomentIdeal(q); rad2deg(theta_total(q))];
%     end
% end
%ToDo Prove this works
% for q = 2:length(theta_orange)
%     if((theta_orange(q)-theta_total(q-1))<0) 
%         slope_orange_error = [theta_orange(q)];
%     end
% end
...
%% Dynamically Adjust Sampling
%8 million points with constant res worked
% TA_slope_orange = dydx(OrangeMomentIdeal,theta_total);
% %high_res = 0.00001;
% hs_index = 1;
% theta_resampled = [];
% gain = 100;
% figure(15)
% hold on
% %plot(rad2deg(theta_total),TA_slope_orange)
% % slope_check = TA_slope_orange>500;
% for i = 1:(length(TA_slope_orange)-1)
%     i;
%     if TA_slope_orange(i)>500
%         high_res = gain*TA_slope_orange(i);
%         inc = (theta_total(i)-theta_total(i-1))/high_res;
%         resampled = [theta_total(i-1):inc:theta_total(i)];
%         theta_resampled = [theta_resampled theta_total(hs_index:i-2) resampled theta_total(i+1)];
%         hs_index = i;
%     end
% end
% theta_resampled = [theta_resampled theta_total(hs_index:end)];
% %plot(rad2deg(theta_resampled))
% OrangeMomentIdeal = interp1(theta_total,OrangeMomentIdeal,theta_resampled,'spline');
% BlueMomentIdeal = interp1(theta_total,BlueMomentIdeal,theta_resampled,'spline');
% theta_total = theta_resampled;
% % plot(rad2deg(theta_total),OrangeMomentIdeal)
% % plot(rad2deg(theta_total),BlueMomentIdeal)
% % plot(theta_total)
...
if(orange_preloaded)
    ...
    gamma2o = preload;
    gammao = sqrt((-ES+0.5*k*gamma2o.^2)/(0.5*k))-gamma2o;
end
...
angle_constant_fw2 = angle_constant_fw+ gamma2o; %ToDo: argue with Hashim equivalent to beta
...
% figure(18)
% hold on
% plot(rad2deg(omega_fw2),'linewidth',2)
% hold off
...
% xprime2 = dydx(x2,psi2);
% yprime2 = dydx(y2,psi2);
...
%% CAM CURVE SELF INTERSECTIONS? (First check for nonrealizable geometry)
...
plot(rad2deg(theta_total),stiffness_instant_orange)
plot(rad2deg(theta_total),stiffness_instant_blue)

%% ToDo Helping Emily debug feel free to delete future self

% psii2 = psi2(1):(psi2(end)-psi2(1))/64000:psi2(end);
% ri2 = interp1(psi2, r2, psii2,'pchip'); 
% 
% 
% isreal(psi2)
...
if(plot_select)
    x_center_new = perc2mm([0 50 100],x_center_max,x_center_min); %same as [0, 33, 45]-33;
...
%     OrangeMomentAnkle = dydx(work_spring_inv2,theta_cam_inv2);
    ...
    if(x_center_new(i)==x_center)
        DropFoot_blue_index = find(abs(BlueMomentAnkle-Torque_DropFoot)<0.02);
        Angle_DropFoot_blue = rad2deg(theta_inv(DropFoot_blue_index(1)))
        DropFoot_orange_index = find(abs(OrangeMomentAnkle-Torque_DropFoot)<0.02);
...
% stiffness = (12.507+0.32492)./deg2rad(11.9301+6.83);
% E_D = 2.5384;
% k_rot_max = (2257.2^(-1)+350^(-1))^(-1)
% deg_sat = rad2deg(12.507/k_rot_max)
% E_max = 12.507*deg2rad(11.9301+6.83-deg_sat)

%% Investigating unreasonable slope
TA_slope_orange_forward = dydx(OrangeMomentIdeal,theta_total);
TA_slope_orange_inverse = dydx(OrangeMomentAnkle,theta_inv2);
figure(18)
hold on
plot(rad2deg(theta_total),TA_slope_orange_forward)
plot(rad2deg(theta_inv2),TA_slope_orange_inverse)
...
%% r,psi as an anaylitical function
% close all
% mid = find(r==min(r));
% band = round(0.05*(length(r)-mid));
% index_chunks = [1; mid-band; mid; mid+band; length(r)];
% order = [7 4 4 7];
% for i=2:5
%     split_index = index_chunks(i);
%     psi_chunk = psi(index_chunks(i-1):index_chunks(i));
%     r_chunk = r(index_chunks(i-1):index_chunks(i));
%     %Fits
%     r_psi_fit = polyfit(psi_chunk,r_chunk,order(i-1));
%     dr_fit = polyder(r_psi_fit);
%     ddr_fit = polyder(dr_fit);
%     %Vectors
%     r_fit = polyval(r_psi_fit,psi_chunk);
%     dr = polyval(dr_fit,psi_chunk);
%     ddr = polyval(ddr_fit,psi_chunk);
%     %-----------------------
%     dr_discrete = dydx(r_chunk,psi_chunk);
%     ddr_discrete = dydx(dr_discrete,psi_chunk);
% 
%     %Radius of curvature
%     R_discrete = (r_chunk.^2 + dr_discrete.^2).^(3/2)./(r_chunk.^2 + 2*dr_discrete.^2 - r_chunk.*ddr_discrete);
%     R = (r_fit.^2 + dr.^2).^(3/2)./((r_fit).^2 + 2*dr.^2 - r_fit.*ddr);
% 
% 
...
%     subplot(2,2,1)
%     hold on
%     plot(psi_chunk,r_fit,'linewidth',5)
%     plot(psi_chunk,r_chunk,'linewidth',2)
%     legend('polynomial function','discrete vector')
%     legend box off
% 
%     subplot(2,2,2)
%     hold on
%     plot(psi_chunk,dr,'linewidth',5)
%     plot(psi_chunk,dr_discrete,'linewidth',2)
%     legend('polynomial function','discrete vector')
%     legend box off
% 
%     subplot(2,2,3)
%     hold on
%     plot(psi_chunk,ddr,'linewidth',5)
%     plot(psi_chunk,ddr_discrete,'linewidth',2)
%     legend('polynomial function','discrete vector')
%     legend box off
% 
%     %Check if fit is accurate enough
%     radius_diff_mm = max(abs(r_chunk-r_fit))*m2mm
%     if(abs(radius_diff_mm)<0.010)
%         disp('Success: Non Machinable Difference')
%     else
%         disp('Error: Machinable Difference')
%     end
% 
%     %Check everything visually
%     subplot(2,2,4)
%     hold on
%     plot(psi_chunk,R*m2mm,'linewidth',5)
%     %plot(psi_chunk,R_discrete*m2mm,'linewidth',2)
%     legend('polynomial function','discrete vector')
%     legend box off
% end
% 
% %psi_poly = min(psi):res*mm2m:max(psi_chunk);
% %x_poly = r_poly.*cos(psi_chunk);
% %y = r_fit.*sin(psi_chunk);
...
%%
    ...
    % equilibrium_cam = 0;
    % if(energy_cam)
    %     if(pushoff_energy>dorsi_energy)
    %         blue_first = 1;
    %         orange_first = 0;
    %         dorsi_energy = dorsi_energy;
    %         midstance_energy = midstance_energy;
    %     else
    %         blue_first = 0;
    %         orange_first = 1;
    %         dorsi_energy = pushoff_energy;
    %         midstance_energy = -midstance_energy;
    %     end
    % end
    % if(equilibrium_cam)
    %     if(Angle_DropFoot_blue>Angle_DropFoot_orange)
    %         blue_first = 1;
    %         orange_first = 0;
    %         dorsi_energy = dorsi_energy;
