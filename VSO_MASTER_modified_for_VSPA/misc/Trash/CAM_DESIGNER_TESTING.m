...
primary_curve_scaled = 1; %plots scaled primary curve
HUMAN = 1;
Collins = 0;
scale_Collins = 0.6;
AFO = 0;
...
if((cam_lin+cam_nonlin+cam_feather+cam_children)>1)
...
if(cam_feather)
    ...
    cam_radius = 0.008; %(meters) %Cam roller radius
    %cam_radius = 0.014; %(meters) %Cam roller radius
end
...
if(spring_feather)
    ...
    %k_spring = [0.219364464676160,0.317555691627756,0.469565769542144,0.690246150791957,1.04760469982981,1.59882829715170,2.32430571880767]*10^6; %slightly updated, but mostly the same as the one below
...
if(characterization_parameter_testing)
    ...
    %Series Compliance
    if(kdelt_nonlinear)
        kdelt_dorsi = 700; % 500 used for ramtech cams 4, 5, and 6 (450)
        kdelt_plantar = 300; %300 used for ramtech cams 4, 5, and 6 (200)
        kdelt = kdelt_dorsi*(theta_total>0)+kdelt_plantar*~(theta_total>0);
    end
end
